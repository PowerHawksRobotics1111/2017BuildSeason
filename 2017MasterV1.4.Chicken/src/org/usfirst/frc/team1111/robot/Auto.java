...
//		
		...
		if (findPegDist && !movePegDist)
		{
			...
			double distanceDelta = targetMovementDist + ((Math.abs(Motors.motorDriveLeft1.getEncPosition())
					+ Math.abs(Motors.motorDriveRight1.getEncPosition())) / 2.0);
			SmartDashboard.putNumber("distanceDelta Retrieval Peg", distanceDelta);
			if ((distanceDelta > -EncoderDistanceThreshold) && (distanceDelta < EncoderDistanceThreshold))
			{
				movePegDist = true;
				Drivetrain.drive(0, 0);
	...
	public static void highBoilerEasy(boolean red)
	{
		...
		if(!shootHigh)
		{
			...
			Fuel.autoShoot();
			if(Math.abs(startTime - timer.get()) >= 1.0)
			{
				Motors.fuelStop.setAngle(Motors.fuelOpenAngle);	
			}
			if(Math.abs(startTime - timer.get()) >= 2.5)
			{
				Motors.motorIntake.set(Motors.intakePower);	
			}
			if (Math.abs(startTime - timer.get()) >= 7.0)
			{
				Fuel.stopShoot();
